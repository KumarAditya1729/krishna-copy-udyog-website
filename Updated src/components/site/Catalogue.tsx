...
export function Catalogue() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isValid) {
      const message = `Hello ${SITE.name}, I want bulk pricing for: ${form.category}. Quantity: ${form.quantity}, Location: ${form.location}, Name: ${form.name}, Phone: ${form.phone}`;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSubmit} className="mt-10">
          <div className="grid grid-cols-2 gap-6">
            <div>
              <label htmlFor="category" className="block text-sm font-medium text-accent">Category</label>
              <input
                id="category"
                name="category"
                type="text"
                ...
                className={`mt-1 block w-full rounded-md border border-border py-2 px-3 text-base placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary-foreground sm:text-sm`}
              />
              {errors.category && <p className="mt-1 text-red-500">{errors.category}</p>}
            </div>
            <div>
              <label htmlFor="quantity" className="block text-sm font-medium text-accent">Quantity</label>
              <input
                id="quantity"
                name="quantity"
                type="text"
                ...
                className={`mt-1 block w-full rounded-md border border-border py-2 px-3 text-base placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary-foreground sm:text-sm`}
              />
              {errors.quantity && <p className="mt-1 text-red-500">{errors.quantity}</p>}
            </div>
            <div>
              <label htmlFor="location" className="block text-sm font-medium text-accent">Delivery Location</label>
              <input
                id="location"
                name="location"
                type="text"
                ...
                className={`mt-1 block w-full rounded-md border border-border py-2 px-3 text-base placeholder:text-muted-foreground focus:border-primary focus:ring-2 focus:ring-primary-foreground sm:text-sm`}
              />
              {errors.location && <p className="mt-1 text-red-500">{errors.location}</p>}
            </div>
            <div>
            ...
            <div>
              <label htmlFor="phone" className="block text-sm font-medium text-accent">Phone</label>
              <input
                id="phone"
                name="phone"
                type="text"
          ...
          <button
            type="submit"
            className="mt-6 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white hover:bg-primary/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-foreground"
          >
        ...
        <div className="mt-6">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-success text-white hover:bg-success/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-success-foreground"
          >
